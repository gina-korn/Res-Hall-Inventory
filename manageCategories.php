<?php
	...
	/////// Forms ////////////
			...
			do 
			{
				if ($result = $mysqli->store_result()) 
				{
					...
					while ($row = $result->fetch_row()) 
					{
						echo '
						<tr bgcolor="' . $bg . '">
							<td width="260"><b>' . $row[1] . '</b></td>
							<td valign="right">
								<button class="buttons" name="catEdit"	value="' . $row[0] . '">
									<img src="./images/edit.png" length="20" width="20" /></button>
								<button class="buttons" name="catDelete" value="' . $row[0] . '" onclick="return confirmation()">
		...
		//Take this out once all stored procedures are implemented
		mysqli_close($dbc);
